...
class ResponsiveCaptchaStrings {
    public static $numberNames = [
        0   => "zero",
        1   => "un",
        2   => "deux",
        3   => "trois",
    ...
    public static $OrString =                   ' ou ';
    public static $AndString =                  ' et ';
    public static $PlusString =                 ' plus ';
    public static $MinusString =                ' moins ';
    public static $TimesString =                ' fois ';
    public static $ByString =                   ' par ';
    public static $LastString =                 'dernière';
    public static $InTheWordString =            ' lettre dans le mot ';
    public static $CannotStartSession =         'Impossible de démarrer la session';
    public static $CaptchaNotYetGenerated =     'Aucun CAPTCHA n\'a été généré';
    public static $WhatIsTheSumOf =             'Quelle est la somme de ';
    public static $WhatIsAPlusB =               'Combien font ';
    public static $WhatIsAMinusB =              'Combien font ';
    public static $WhatIsATimesB =              'Combien font ';
    public static $WhatIsAMultipliedByB =       'Quel est le produit de ';
    public static $WhatIsADividedByB =          'Quel est le résultat de la division de ';
    public static $WhatIsTheLetterPosInWord =   'Quelle est la ';
    public static $WhichIsTheSmallest =         'Quel est le plus petit : ';
    public static $WhichIsTheLargest =          'Quel est le plus grand : ';
    public static $NumberMustBeAnInteger =      'Le nombre doit être un entier';
    public static $NumberMustBeBetween1And100 = 'Le nombre doit être entre 1 et 100';
}
...
class ResponsiveCaptcha
{
    ...
    private function startSession()
    {
        if (session_status() === PHP_SESSION_NONE) {
            ...
            if (!session_start()) {
                throw new \Exception(ResponsiveCaptchaStrings::$CannotStartSession);
            } else {
    ...
    public function getAnswer()
    {
        ...
        if (!isset($_SESSION[$this->sessionVariable])) {
            throw new \Exception(ResponsiveCaptchaStrings::$CaptchaNotYetGenerated);
        }
    ...
    private function getAdditionProblem()
    {
        ...
        if (rand(0, 1)) {
            return ResponsiveCaptchaStrings::$WhatIsTheSumOf . $num1Name . ResponsiveCaptchaStrings::$AndString . $num2Name . " ?";
        } else {
            return ResponsiveCaptchaStrings::$WhatIsAPlusB .  $num1Name . ResponsiveCaptchaStrings::$PlusString . $num2Name . " ?";
        }
    }
    ...
    private function getSubtractionProblem()
    {
        ...
        return ResponsiveCaptchaStrings::$WhatIsAMinusB . $largerNumberName . ResponsiveCaptchaStrings::$MinusString . $smallerNumberName . " ?";
    }
    ...
    private function getMultiplicationProblem()
    {
        ...
        if (rand(0, 1))
            return ResponsiveCaptchaStrings::$WhatIsAMultipliedByB . $num1Name . ResponsiveCaptchaStrings::$AndString . $num2Name . " ?";
        else
            return ResponsiveCaptchaStrings::$WhatIsATimesB . $num1Name . ResponsiveCaptchaStrings::$TimesString . $num2Name . " ?";
    }
    ...
    private function getDivisionProblem()
    {
        ...
        return ResponsiveCaptchaStrings::$WhatIsADividedByB . $dividendName . ResponsiveCaptchaStrings::$ByString . $divisorName . " ?";
    }
    ...
    {
        $words = ResponsiveCaptchaStrings::$words;
        $numberNames = ResponsiveCaptchaStrings::$numberPos;
        ...
        if (rand(1, $randomWordLength) == $randomWordLength) {
            $letterPosName = ResponsiveCaptchaStrings::$LastString;
        ...
        return ResponsiveCaptchaStrings::$WhatIsTheLetterPosInWord .  $letterPosName . ResponsiveCaptchaStrings::$InTheWordString . $randomWord . " ?";
    }
    ...
    private function getNumberProblem()
    {
        ...
        for ($i = 0; $i < count($numbers); $i++) {
            ...
            if ($i == count($numbers) - 1) {
                // the last number
                $numberString .= ResponsiveCaptchaStrings::$OrString . " $numberName";
            } else
        ...
        if (rand(0, 1)) {
            ...
            return ResponsiveCaptchaStrings::$WhichIsTheSmallest . $numberString . " ?";
        } else {
            // ask which is largest
            rsort($numbers); // so the first element contains the largest number
            $this->storeAnswer($numbers[0]);
            return ResponsiveCaptchaStrings::$WhichIsTheLargest . $numberString . " ?";
        }
    }
    ...
    {
        if (!is_int($number)) {
            throw new \InvalidArgumentException(ResponsiveCaptchaStrings::$NumberMustBeAnInteger);
        } elseif ($number < 0 || $number > 100) {
            throw new \UnexpectedValueException(ResponsiveCaptchaStrings::$NumberMustBeBetween1And100);
        }

        $numberNames = ResponsiveCaptchaStrings::$numberNames;

        if (array_key_exists($number, $numberNames)) {
